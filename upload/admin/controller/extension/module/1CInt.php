<?php
class Controllerextensionmodule1CInt extends Controller {
    private $error = array();

    public function index() {
        $this->load->language('module/1CInt'); //подключаем наш языковой файл
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) { //если мы нажали "Сохранить"  в панели, мы сохраняем текущие настройки
            $this->model_setting_setting->editSetting('1CInt', $this->request->post);
            $this->response->redirect($this->url->link('extension/1CInt', 'token=' . $this->session->data['token'], 'SSL'));
        }
        ...
        $data['action'] = $this->url->link('module/1CInt', 'token=' . $this->session->data['token'], 'SSL');
        $data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

        //переменная с статусом модуля
        if (isset($this->request->post['1CInt_status'])) {
            $data['1CInt_status'] = $this->request->post['1CInt_status'];
        } else {
            $data['1CInt_status'] = $this->config->get('1CInt_status');
        }
        ...
        $this->response->setOutput($this->load->view('module/1CInt.tpl', $data));
    }
